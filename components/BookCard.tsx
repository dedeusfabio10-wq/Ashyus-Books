...
const BookCard: React.FC<BookCardProps> = ({ book }) => {
    return (
        <div className="group bg-slate-800/40 backdrop-blur-sm border border-white/5 rounded-xl overflow-hidden shadow-xl shadow-black/20 hover:shadow-brand-gold/10 hover:border-brand-gold/30 transform hover:-translate-y-2 transition-all duration-500 flex flex-col animate-fade-in">
            ...
            <div className="p-6 flex flex-col flex-grow relative">
                <h3 className="font-serif text-2xl font-bold text-brand-lighter mb-2 leading-tight group-hover:text-brand-gold transition-colors">{book.title}</h3>
                <p className="text-gray-400 text-sm mb-6 flex-grow leading-relaxed line-clamp-4">{book.shortSynopsis}</p>
